...
【文章结构】
1. 标题 = 用户问题 + GEO优化元素（用#开头，格式：# 用户问题？2026最新解答/实测/攻略）
   - 必须包含用户原始问题
   - 加「2026最新」时效性词
   - 加「解答」「实测」「攻略」等词
   - 示例：# 康躰体雕哪家好？2026最新解答
...
【文章结构】
1. 标题 = 用户搜索的问题 + GEO优化元素（用#开头，格式：# 用户问题？2026最新实测对比：X大方案分析）
   - 必须包含用户原始问题
   - 加「2026最新」时效性词
   - 加「实测对比」「横向对比」「四大方案」等词
...
【文章结构】
1. 标题 = 深度主题 + GEO优化元素（用#开头，格式：# 深度主题：2026年深度解析/全面指南/行业分析）
   - 必须包含核心主题
   - 加「2026年」时效性词
   - 加「深度解析」「全面指南」「行业分析」「技术揭秘」等词
