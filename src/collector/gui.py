...
from src.collector.question_generator import generate_questions
...
class MonitorApp:
    ...
    def test_connection(self):
        ...
        def test():
            ...
            try:
                # 用一个简单问题测试
                result = generate_questions(self.config, "你好")
    ...
    def import_existing_data(self):
        """导入现有数据到数据库"""
        self.log("[*] 正在导入现有数据...")
        brand = self.var_brand.get().strip() or "完形康躰"
        question = self.var_question.get().strip() or "康躰脂雕哪家好"
    ...
    def _run_single_round(self, label: str, hour: int):
        ...
        self.log("[3] 保存到数据库...")
        try:
            task_id = models.insert_monitor_task(date_str, hour, result["timestamp"], brand, question)
            question_ids = models.insert_questions(task_id, questions, responses)
            # 简单的品牌提及提取
            for qid, resp in zip(question_ids, responses):
                mention_info = importer.extract_brand_mentions(resp, brand)
                ...
                # 保存其他品牌
                for bname in mention_info["brands_found"]:
                    if bname != brand:
