...
| `llm_api.provider` | API 提供商 | `qwen` / `openai` / `claude` / `doubao` |
| `llm_api.model` | 模型名称 | `qwen3.6-flash` |
| `llm_api.base_url` | API 地址 | 根据你的 provider 填写 |
...
可以。在 GUI 界面直接切换，或修改 `config.yaml` 中的 `llm_api` 部分。支持的 provider：
